...
Source expressions are not interpolation templates. A value such as
`field:title - Site Name` is invalid because everything after `field:` is
parsed as the field specification. Keep the source as `field:title` and add
shared title text with Ichiban's Title Format setting, for example
